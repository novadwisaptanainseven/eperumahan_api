...
class Perumahan extends Model
{
    ...
    public static function addPerumahanMaster($req)
    {
        ...
        $data_perumahan = [
            ...
            "tahun"               => $req->tahun,
            "luas"                => $req->luas,
            "luas_mbr"            => $req->luas_mbr,
            "luas_komersial"      => $req->luas_komersial,
            "jumlah_unit_mbr"     => $req->jumlah_unit_mbr,
            "jumlah_unit_non_mbr" => $req->jumlah_unit_non_mbr,
            "jumlah_unit"         => $req->jumlah_unit,
            "jumlah_ruko"         => $req->jumlah_ruko,
            "keterangan_unit"     => $req->keterangan_unit,
        ...
        if ($req->sarana_prasarana_perumahan) {
    ...
    public static function updatePerumahanMaster($req, $id_perumahan)
    {
        ...
        if ($data_perumahan) {
            ...
            $data = [
                ...
                'luas_mbr' => ($req->luas_mbr) ? $req->luas_mbr : $data_perumahan->luas_mbr,
                'luas_komersial' => ($req->luas_komersial) ? $req->luas_komersial : $data_perumahan->luas_komersial,
                ...
                'keterangan_unit' => ($req->keterangan_unit) ? $req->keterangan_unit : $data_perumahan->keterangan_unit,
                'jumlah_ruko' => ($req->jumlah_ruko) ? $req->jumlah_ruko : $data_perumahan->jumlah_ruko,
    ...
    public static function searchPerumahanByValue($search_value)
    {
        ...
        $data_perumahan = DB::table($perumahan)
            ...
            ->orWhere('tahun', 'like', "%$search_value%")
            ->orWhere('nama_kategori', 'like', "%$search_value%")
    ...
    public static function addProperti($req, $id_perumahan)
    {
        ...
        if (!$req->hasFile('denah'))
            ...
            $denah = $file->storeAs("bangunan/file", rand(0, 9999) . '-' . date('Ymd') . '-' . $sanitize);
        }

        if (!$req->hasFile('sertifikat'))
            $sertifikat = '';
        else {
            ...
            $sertifikat = $file->storeAs("bangunan/file", rand(0, 9999) . '-' . date('Ymd') . '-' . $sanitize);
        }
    ...
    public static function updatePropertiById($req, $id_perumahan, $id_bangunan)
    {
        ...
        // Persiapan data
        // Cek apakah ada file denah
        if (!$req->hasFile('denah')) {
            $denah = $data_bangunan->denah;
        } else {
            $file = $req->file('denah');

            // Hapus file denah lama jika user update file denah
            // Get path file denah untuk keperluan menghapus file denah di storage
            $path_denah = $data_bangunan->denah;
            Storage::delete("$path_denah");
        ...
        if (!$req->hasFile('sertifikat')) {
            $sertifikat = $data_bangunan->sertifikat;
        } else {
            $file = $req->file('sertifikat');

            // Hapus file sertifikat lama jika user update file sertifikat
            // Get path file sertifikat untuk keperluan menghapus file sertifikat di storage
            $path_sertifikat = $data_bangunan->sertifikat;
            Storage::delete("$path_sertifikat");
    ...
    public static function getAllPerumahanExport($req)
    {
        ...
        if ($orderBy && $kategori) {
            $perumahan = Perumahan::where("$tblPerumahan.id_kategori", $kategori)
                ->leftJoin($tblKategori, "$tblKategori.id_kategori", "=", "$tblPerumahan.id_kategori")
                ->leftJoin($tblPengembang, "$tblPengembang.id_pengembang", "=", "$tblPerumahan.id_pengembang")
                ->orderBy("$tblPerumahan.$orderBy", $order)
                ->get();
        } elseif ($orderBy) {
    ...
    // Get All Perumahan Verifikasi 
    public static function getAllPerumahanVerifikasiExport($req)
    {
        $tblPerumahan = "perumahan";
        $tblPengembang = "pengembang";
        $tblKategori = "kategori";
        ...
        if ($status == 1 || $status == 2) {
            $perumahan = Perumahan::where("$tblPerumahan.status_perumahan", 1)
                ->orWhere("status_perumahan", 2)
                ->leftJoin($tblKategori, "$tblKategori.id_kategori", "=", "$tblPerumahan.id_kategori")
                ->leftJoin($tblPengembang, "$tblPengembang.id_pengembang", "=", "$tblPerumahan.id_pengembang")
                ->orderBy("$tblPerumahan.created_at", $order)
                ->get();
            ...
            $perumahan = Perumahan::leftJoin($tblKategori, "$tblKategori.id_kategori", "=", "$tblPerumahan.id_kategori")
                ->leftJoin($tblPengembang, "$tblPengembang.id_pengembang", "=", "$tblPerumahan.id_pengembang")
                ->orderBy("$tblPerumahan.status_perumahan", $order)
                ->get();
        }
